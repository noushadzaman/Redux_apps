...
export default function Todo({ t }) {
    ...
    const { text, id, completed, color } = t;

    function handleStatusChange(todoId) {
        dispatch(toggled(todoId));
    }

    function handleColorChange(todoId, color) {
    ...
    return (
        <div className={`flex justify-start items-center p-2 hover:bg-gray-100 hover:transition-all space-x-4 border-b border-gray-400/20 last:border-0`}>
            ...
            <div
                ...
                className={`flex-shrink-0 h-4 w-4 rounded-full border-2 ml-auto cursor-pointer hover:bg-red-500 border-red-500 ${color === 'red' && 'bg-red-500'}`}></div>
